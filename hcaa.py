# https://github.com/hudson-and-thames/mlfinlab/blob/master/mlfinlab/portfolio_optimization/hcaa.py
    ...
    def _recursive_bisection(self,
        ...
        while clustered_alphas:
            ...
            for subcluster in range(0, len(clustered_alphas), 2):
                ...
                # https://residualmetrics.com/index.php/featured-home/10-finance-markets/39-testing-the-performance-of-hierarchical-risk-parity-for-portfolio-optimisation-using-jse-shares
                wl = self.weights[left_cluster[0]]
                wr = self.weights[right_cluster[0]]
                alloc_factor = np.minimum(sum([max_weight] * len(left_cluster)) / wl, np.maximum(sum([min_weight] * len(left_cluster)) / wl, alloc_factor))
                alloc_factor = 1 - np.minimum(sum([max_weight] * len(right_cluster)) / wr, np.maximum(sum([min_weight] * len(right_cluster)) / wr, 1 - alloc_factor))
